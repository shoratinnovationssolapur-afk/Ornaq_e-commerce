...
export default function ProductCard({ product, dark = false, onCardClick }) {
  ...
  const cardContent = (
    <div className="relative block aspect-[4/5] overflow-hidden bg-stone-50">
        <img
        ...
        <div className="mt-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-black text-stone-900">{formatCurrency(offerPrice)}</span>
              {showOffer && <span className="text-xs font-bold text-stone-300 line-through">{formatCurrency(marketPrice)}</span>}
            </div>
            {colors.length > 0 && (
